...
export function usePointerDrag(actor: Actor) {
  ...
  const onPointerMove = (e: React.PointerEvent<HTMLElement>) => {
    ...
    if (Math.abs(e.clientX - st.sx) > 5 || Math.abs(e.clientY - st.sy) > 5)
      st.moved = true;
    // セットプレー文書ではライン上・コーナーに置けることが本質（CKのボールはコーナーアーク、
    // ポスト番の守備者はゴールライン上）なので、クランプを盤端ぎりぎりまで緩める。
    // 戦術ボード(setPieceなし)は従来どおりの余白つきクランプのまま
    const sp = board.stateRef.current.setPiece != null;
    const isBall = actor === "ball";
