...
export default function ConsoleShell({ children }: { children: React.ReactNode }) {
  ...
  const entries: Entry[] = coach
    ? [
        ...
        { sec: "その他", item: { key: "articles", label: "お役立ち記事", icon: <IconBook />, onSelect: () => board.openSheet({ type: "articles" }) } },
        { item: { key: "settings", label: "設定", icon: <IconCog />, onSelect: () => board.openSheet({ type: "settings" }) } },
      ]
    : [
        { item: { key: "home", label: "ホーム", icon: <IconHome />, onSelect: () => board.setScreen("home") } },
