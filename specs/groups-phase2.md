...
保存 key は **`"board"` の 1 本**（配置シート・ボードの名簿シート・ベンチで共有）。単一選択・`allowAll` あり（`allLabel` は既定の「すべて」）、`onManage` は付けない。

> review #2 回目で更新：実装（他画面の matches/notebook/chat と挙動を揃えるため `allowAll` を追加）に合わせて仕様側を直した。
...
- 選手を選ぶ select の候補（スタメン各枠・得点者・アシスト・交代 OUT/IN のすべて）：
  ...
  - 各 select の option = `base` ∪ **その select で現在選ばれている選手**（末尾に足し、ラベルの後ろに「・外」。スマホの select は幅 146px 前後で「（対象外）」だと名前ごと切れるため短い印にした）。選択済みの値が候補から消えて表示と state がずれる事故を防ぐ。
...
2. 試合記録：Aチーム対象の予定から「この試合の結果を記録」→ 対象に Aチームが入り、スタメンの候補が Aチームの選手だけ。「対象外の選手も候補に出す」で 70 人。選んだ後に対象を変えても選択は消えず「・外」付きで残る。保存後、一覧で Aチームを選ぶとその記録と全体の記録が出て、Bチームではその記録が出ない。
