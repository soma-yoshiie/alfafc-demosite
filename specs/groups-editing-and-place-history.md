...
- `soccer_tactics_state_v1` の `players` に、旧サンプルの選手（`id` が `p01`〜`p16`）が **8 人以上** 残っている。自分で足した選手やチーム名の変更があっても対象（実チームは未運用のため、旧サンプルが残っていれば「旧デモを使い続けているブラウザ」とみなす）。
- `soccer_tactics_team_v1` が無い、または `schoolStage` が未設定か `"elementary"`。

> 2026-09-18 改訂：当初は「人数 16 以下・全員が旧サンプル id・チーム名が旧名」の厳しい署名だったが、数週間デモを触って選手を足したりチーム名を変えたりしたブラウザでは一度も移行されず、マーカー `"2"` が書かれて以後は再判定もされなかった。判定を緩め、マーカーの版を `"3"` に上げて判定し直す（`DEMO_SEED_VERSION`）。自分で足した選手（サンプル外の id）は末尾に残し、小学生の学年（4 以上）は未設定に戻す。変更したチーム名は残す。
> あわせて設定に **「デモデータを入れ直す」**（保存データをすべて消して初回起動と同じ新しいデモに戻す。ログイン情報は残す。`window.confirm` で確認）を置き、判定に掛からない場合の逃げ道にする（`lib/storage.ts` の `resetAppData`）。
